...
DEV = _os_dev.environ.get("EXPORT_DEV") or ("mps" if __import__("torch").backends.mps.is_available() else "cpu")
...
def quantize(weight, chunk=16384):
    ...
    if out_dim <= 20000:                                   # transformer layers: fast single-shot GPU path
        return _quant_block(weight.t().contiguous().float().to(DEV))
    ...
    for s in range(0, out_dim, chunk):
        p, c, d = _quant_block(Wt_full[:, s:s + chunk].to(DEV))
        packs.append(p); cbs.append(c); dqs.append(d)
        (__import__('torch').mps.empty_cache() if DEV=='mps' else None)
...
def main():
    ...
    ).to(DEV)  # device-agnostic (cpu/mps); TinyLlama-1.1B fits easily
    cfg = model.config
