...
fn check(m: usize) -> f32 {
    ...
    // CPU reference: W[o, g*D+e] = scale[o] * sum_m CB[mi][codes[mi][g][o]][e]
        ...
        for g in 0..ng {
            for e in 0..AVQ_D {
                ...
                for mi in 0..m {
                    ...
                    w += cb[mi * AVQ_K * AVQ_D + code * AVQ_D + e];
                }
                acc += scale[o] * w * x[g * AVQ_D + e];
            }
        }
        y_ref[o] = acc;
    }
...
fn main() {
    for m in [2usize, 3] {
        let err = check(m);
        let ok = err < 2e-2; // fp16 activation quantization dominates the tolerance
