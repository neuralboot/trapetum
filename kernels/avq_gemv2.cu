...
// count from ~3 per group (v1) to 2 per block, the main reason v1 sat far below the
// memory roofline.
//
// build:  nvcc -O3 -arch=sm_86 -DGT=8 avq_gemv2.cu -lcublas -o avq2 && ./avq2
#include <cuda_fp16.h>
#include <cublas_v2.h>
#include <cstdio>
#include <cstdlib>
...
#define CPB 256
#ifndef GT
#define GT 8          // groups per block (group tile)
#endif

__global__ void avq_gemv2(const __half* __restrict__ X, const unsigned char* __restrict__ codes,
    ...
    __syncthreads();
    for (int t = threadIdx.x; t < M*GT*K; t += CPB) {        // LUT[m][gt][k] = <x_{g0+gt}, C_m[k]>
    ...
    if (o < OC) {
        ...
        for (int gt = 0; gt < GT; gt++) {
            ...
            #pragma unroll
            for (int m = 0; m < M; m++)
                acc += s_LUT[(m*GT + gt)*K + codes[((size_t)m*ng + g)*OC + o]];
        }
        atomicAdd(&Y[o], acc);
    }
}
...
int main() {
    ...
    dim3 grid(OC/CPB, (ng+GT-1)/GT), block(CPB);
    cudaMemset(Y,0,OC*4); avq_gemv2<<<grid,block>>>(X,codes,CB,Y,IC,OC); cudaDeviceSynchronize();
    ...
    for (int i=0;i<30;i++){cudaMemset(Y,0,OC*4); avq_gemv2<<<grid,block>>>(X,codes,CB,Y,IC,OC);}
    cudaDeviceSynchronize(); cudaEventRecord(a);
    for (int i=0;i<it;i++){cudaMemset(Y,0,OC*4); avq_gemv2<<<grid,block>>>(X,codes,CB,Y,IC,OC);}
    ...
    double codesMB=(double)M*ng*OC/1e6;
    cudaDeviceProp pr; cudaGetDeviceProperties(&pr,0);
    printf("%s GT=%d : avq2 %.4f ms | cuBLAS %.4f ms | x%.2f | codes %.1fMB | eff %.0f GB/s\n",
           pr.name, GT, t_avq, t_cub, t_cub/t_avq, codesMB, codesMB/t_avq);
    return 0;
}
